...
function wpeppsubNCX_plugin_buttons() {
	...
	if (!isset($_GET['action']) || $_GET['action'] == "delete" || !empty($_GET['action2']) == "delete") {
		...
		class wpeppsubNCX_buttons_table extends WP_List_Table {
			...
			function get_data() {
				...
				foreach ($posts as $post) {
					...
					$shortcode = 	'<input type="text" readonly="true" size="40" value="[wpeppsubNCX id='.$id.']">';
			...
			function column_product($item){
				...
				// edit
				$edit_bare = '?page=wpeppsubNCX_buttons&action=edit&product='.$item['ID'];
				$edit_url = wp_nonce_url($edit_bare, 'edit_'.$item['ID']);
				
				// delete
				$delete_bare = '?page=wpeppsubNCX_buttons&action=delete&inline=true&product='.$item['ID'];
		...
		function wpeppsubNCX_tt_render_list_page() {

			$testListTable = new wpeppsubNCX_buttons_table();
			...
			<div style="width:98%">
				...
				<a href="?page=wpeppsubNCX_buttons&action=new" name='btn2' class='button-primary' style='font-size: 14px;height: 30px;float: right;'>New Button</a>
				</td></tr></table>
		...
		wpeppsubNCX_tt_render_list_page();
	...
	if (isset($_GET['action']) && $_GET['action'] == "delete" || isset($_GET['action2']) && $_GET['action2'] == "delete") {
		...
		if (empty($post_id)) {
			echo'<script>window.location="?page=wpeppsubNCX_buttons&message=nothing_deleted"; </script>';
			exit;
		}
		...
		foreach ($post_id as $to_delete) {
			...
			delete_post_meta($to_delete,'wpeppsubNCX_button_sku');
			delete_post_meta($to_delete,'wpeppsubNCX_button_buttonsize');
			delete_post_meta($to_delete,'wpeppsubNCX_a3');
			delete_post_meta($to_delete,'wpeppsubNCX_p3');
			delete_post_meta($to_delete,'wpeppsubNCX_t3');
			delete_post_meta($to_delete,'wpeppsubNCX_srt');
			delete_post_meta($to_delete,'wpeppsubNCX_trial_1');
			delete_post_meta($to_delete,'wpeppsubNCX_a1');
			delete_post_meta($to_delete,'wpeppsubNCX_p1');
			delete_post_meta($to_delete,'wpeppsubNCX_t1');
			
		}
		
		echo'<script>window.location="?page=wpeppsubNCX_buttons&message=deleted"; </script>';
		exit;
	...
	if (isset($_GET['action']) && $_GET['action'] == "-1") {
		...
		echo'<script>window.location="?page=wpeppsubNCX_buttons&message=nothing"; </script>';
	}
